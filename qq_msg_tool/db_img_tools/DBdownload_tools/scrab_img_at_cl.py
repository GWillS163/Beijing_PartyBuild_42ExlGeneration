...
def get_web_link(url, encode='utf-8', verbose=True):
    global file_format
    res = requests.get(url)
    try:
        # html_encoding = re.findall(r"charset=\"(.*)?\" />", res.text)[0].strip("\"")
        html_encoding = re.findall(r"charset=(.*)? />", res.text)[0].strip("\"")
        if 'UTF-8' in html_encoding:
            html_encoding = 'utf-8'
        print('找到的编码为', html_encoding)
        if html_encoding:
            res.encoding = html_encoding
    except Exception as E:
        print('解码出错,使用utf-8', E)
        res.encoding = 'utf-8'
    body = res.text
    ...
    print('####解析完毕####')
    return (url, title, body, output_lst, un_output_lst)
    ...
    while True:
        ...
        try:
            ...
            while True:
                ...
                print(f'第{n}次运行')
                url = input('输入网址>>>')
                url, title, output_lst, un_output_lst = get_web_link(url)
