...
from score_module.models import Score, Rank
...
def login(request):
    if request.method == 'GET':
        return render(request, 'score_module/web/login.html')
...
def upload(request):
    if request.method == 'GET':
        return render(request, 'score_module/web/player_up.html', {'user': request.user, })
    if request.method == 'POST':
        # 接受页面上传的数据
...
def show(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'score_module/web/player_up.html', {'context': context, 'count': count, 'uscore': uscore})
    if request.method == 'POST':
        try:
