        ...
        for ruleP in rule.split("\n"):
            ...
            if scopeStr.strip().isdigit():
                if scopeStr in numOfAnswerDigitalList:
                    ...
                    print("[数字判断]无法处理此规则, 请检查！将处理为0分 :", answer, rule, type)
                    return 0
            ...
            if "全选" in scopeStr:
                ...
                permitQuantity = re.search(r"任选(\d)-?(?:\d)?个", ruleP).groups(0)
                if len(permitQuantity) == 1:
                    if "及以上" in scopeStr:
                        if len(numOfAnswerDigitalList) >= int(permitQuantity[0]):
                            return int(scoreStr.strip("分"))
                    elif len(numOfAnswerDigitalList) == int(permitQuantity[0]):
                        return int(scoreStr.strip("分"))
                elif len(permitQuantity) == 2:
                    if int(permitQuantity[0]) <= len(numOfAnswerDigitalList) <= int(permitQuantity[1]):
                        return int(scoreStr.strip("分"))
            else:
                ...
                print(f"Answer:{answer}\n"
                      ...
                      f"type:{type}" )
                return 0
        print("[不定项选择模块]:不能处理此规则请检查！将处理为0分 :", answer, rule, type)
        return 0
    else:
        print("不能判断此题分数请留意! 将处理为0分 :")
        print(f"Answer:{answer}\n"
              f"rule:{rule},"
              f"type:{type}")
        return 0
